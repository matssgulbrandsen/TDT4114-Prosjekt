...
def hent_metadata_og_lagre():
    """
    Henter metadata for tilgjengelige datasett fra NASA sitt API.
    ...
    Returnerer:
        pd.DataFrame: DataFrame med metadata for alle datasett.
    ...
    # 3. Finn prosjektrot (to nivå opp fra src/Havnivådata/) 
    project_root = Path(__file__).resolve().parents[2]

    # 4. Lag underkatalogen data/Havnivådata og skriv JSON dit
...
def ssh_datasets_listcomp(metadata_path: str, terskel: int = 10000) -> pd.DataFrame:
    ...
    med bare de radene der 'title' inneholder 'SSH' (case-insensitivt)
    og 'tileCount' er over terskel, ved hjelp av list comprehension.
    
    Parametre:
        metadata_path (str): Sti til metadata JSON-fil.
        terskel (int): Minimumsverdi for 'tileCount' (default=10000).
    
    Returnerer:
        pd.DataFrame: Filtrert DataFrame med sammenhengende index.
    """
    # Les inn alle rader som dictionary-liste
    records = pd.read_json(metadata_path, orient="records", lines=True).to_dict("records")
    
    # Bruk list comprehension for å filtrere
    filtered = [
...
def filtrer_ssh_datasets_sql(metadata_path: str, terskel: int = 10000) -> pd.DataFrame:
    """
    Leser metadata fra en JSON-fil og returnerer en DataFrame
    med kun radene der 'title' inneholder 'SSH' (case-insensitivt)
    og 'tileCount' er over terskel, ved hjelp av pandasql (sqldf).
    ...
    # Lag en SQL-spørring (case-insensitive 'SSH' i title)
    query = f"""
        SELECT *
        FROM df
...
def last_ned_nasa_ssh_data(
    ...
    Parametre:
        ds_navn (str): Navn på datasettet (default "NASA_SSH_REF_SIMPLE_GRID_V1_Monthly")
        bbox (str): Geografisk bounding box (default global)
        start (str): Starttid ISO-format
        slutt (str): Sluttid ISO-format
        filnavn (str): Filnavn for lagring (default "havnivaadata.json")

    Returnerer:
        pd.DataFrame: DataFrame med de viktigste statistikk-kolonnene for videre analyse
    """
    # 1. Hent dataen vi ønsker fra API
    base_url = "https://sealevel-nexus.jpl.nasa.gov"
    ...
    # 2. Bearbeider rådata og lager DataFrame
